   ...
   (fn [{:keys [props state]}]
     [:a {:href "javascript:;"
          :style {:padding "1ex" :textDecoration "none"
                  ...
                  :color (if (:hovering? @state) (:link-blue common/colors) "black") }
  ...
  {:get-initial-state
   ...
   (fn [{:keys [state]}]
     [:div {}
      ...
      [:div {:style {:padding "1em" :borderBottom (str "1px solid " (:line-gray common/colors))}}
       [:div {:style {:float "right" :fontSize "70%"}}
        [:span {:style {:marginRight "1ex" :color (:link-blue common/colors)}}
