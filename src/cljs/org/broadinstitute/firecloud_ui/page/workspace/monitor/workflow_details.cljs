...
(defn chart-options [executionCallsCount]
  (clj->js {:colors (list "#04E9E7", "#009DF4", "#0201F4", "#01FC01", "#00C400", "#008C00", "#CCAD51", "#2dd801", "#F99200", "#9854C6", "#F800FD", "#BC0000", "#FD0000")
...
(defn add-to-chart [calldata workflow-name data-table]
  (let [
...
(defn generate-chart [data workflow-name container-id]
  (let [container (.getElementById js/document container-id)
    ...
    (dorun (map #(% (let [reps (add-to-chart % workflow-name data-table)])) data))
   ...
   (fn [{:keys [props state]}]
    (let [workflow-name (workflow-name (:label props))
     ...
     [:div {:style {:marginTop "1em"}}
      ...
      [:div {:style {:display "inline-block" :marginRight ".5em" :width "25vw"}}
        (style/create-link {:text (:label props)
                            :target "_blank"
                            :style {:color "-webkit-link" :textDecoration "underline" :marginRight ".5em"}
          :href (str moncommon/google-cloud-context (:bucketName props) "/" (:submission-id props)
                     "/" workflow-name "/" (:workflowId props) "/" call-name "/")})
      (style/create-link {:text (if (:expanded @state) "Hide" "Show")
                          :onClick #(swap! state assoc :expanded (not (:expanded @state)))})]
      [:div {:style {:display "inline-block" :float "right" :minWidth "70vw"} :id chart-name } "chart"]]
    ...
    :componentDidMount(fn [{:keys [props state]}]
           (let [workflow-name (workflow-name (:label props))
             ...
             (generate-chart (:data props) workflow-name chart-id)
             ))})
