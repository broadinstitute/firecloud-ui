...
(react/defc TrialAlertContainer
  {:render
   (fn [{:keys [state]}]
     (when-not (:dismissed? @state)
       (when-let [current-trial-state (keyword (:trialCurrentState @profile/saved-user-profile))]
         (let [{:keys [title message link button]} (get-in @state [:messages current-trial-state])]
           (flex/box {:style {:color "white" :background-color (:button-primary style/colors)}}
                               ...
                               (when button
                                 (let [{:keys [label url external?]} button]
                                   [:a {:style {:display "block"
                                        ...
                                        :href (if external? url "javascript:;")
                                        :onClick (when-not external?
                                                   (fn []
                                                     (utils/ajax-orch
                                                      "/profile/trial"
                                                      {:method :post
                                                       :on-done #(profile/reload-user-profile)})))
