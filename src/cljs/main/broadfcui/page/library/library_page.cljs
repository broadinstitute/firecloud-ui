                      ...
                      [(when (:show-request-access? @state)
                         (modals/render-message
                          {:header "Request Access"
                           ...
                           :dismiss #(swap! state dissoc :show-request-access? :request-access-message)}))
   ...
   (fn [{:keys [state]} data]
     (let [built-in-groups #{"TCGA-dbGaP-Authorized", "TARGET-dbGaP-Authorized"}
       ...
       (if (= (:workspaceAccess data) "NO ACCESS")
         ...
         {:href (str (when (or common/has-terra-return? common/has-firecloud-return?) (config/terra-url))
