...
(react/defc TextFilter
  {:set-text
   ...
   :render
   (fn [{:keys [props this]}]
     (let [{:keys [initial-text placeholder width data-test-id on-filter]} props]
        ...
        (style/create-search-field
         {:ref "filter-field" :autoSave "true" :results 5 :auto-focus "true"
          ...
          :onKeyDown (common/create-key-handler
                      [:enter :esc]
                      (fn [e]
                        (if (= (.-keyCode e) (:esc common/keymap))
                          (when (seq (.. e -currentTarget -value))
                            (.stopPropagation e) ; prevent closing a modal
                            (this :set-text "")
                            (on-filter ""))
                          (this :apply-filter))))})
