        ...
        (let [original-destination (aget js/window "location" "hash")
          ...
          (case (:registration-status @state)
            ...
            (if component
              ...
              ;;
              ;; commenting out the NPS survey block instead of deleting it, in case we want to reinstate it
              ...
              ]  ;; <-- this ends the [:div {} above. It should be on the previous line, but it should not be commented-out.
              [:h2 {} "Page not found."])))]))
   :component-did-mount
   (fn [{:keys [this state]}]
