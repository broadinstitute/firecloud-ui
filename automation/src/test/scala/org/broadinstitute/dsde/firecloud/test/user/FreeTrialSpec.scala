...
/**
  ...
  */
class FreeTrialSpec extends FreeSpec with Matchers with WebBrowserSpec
  with UserFixtures with CleanUp with LazyLogging with BeforeAndAfterAll {

  var adminUser: Credentials = _
  var campaignManager: Credentials = _
  implicit var authToken: AuthToken = _
  ...
  override def beforeAll {
    super.beforeAll()
    adminUser = UserPool.chooseAdmin
    campaignManager = UserPool.chooseCampaignManager
    authToken = adminUser.makeAuthToken()
