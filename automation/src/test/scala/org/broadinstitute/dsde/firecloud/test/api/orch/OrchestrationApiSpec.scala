  ...
  "Orchestration" - {
    "should grant and remove google role access" in {
      ...
      withBillingProject("auto-goog-role") { projectName =>
        val preRoleFailure = googleBigQueryDAO.startQuery(GoogleProject(projectName), "meh").failed.futureValue
        ...
        val queryReference = eventually {
          val start = googleBigQueryDAO.startQuery(GoogleProject(projectName), shakespeareQuery).futureValue
        ...
        val queryJob = eventually {
          val job = googleBigQueryDAO.getQueryStatus(queryReference).futureValue
          job.getStatus.getState shouldBe "DONE"
          job
        }

        val queryResult = googleBigQueryDAO.getQueryResult(queryJob).futureValue
