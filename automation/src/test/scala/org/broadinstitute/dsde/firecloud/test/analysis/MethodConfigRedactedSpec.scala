...
class MethodConfigRedactedSpec extends FreeSpec with Matchers with WebBrowserSpec with WorkspaceFixtures with UserFixtures with MethodFixtures with BillingFixtures {
  ...
  "For a method config that references a redacted method" - {
    ...
    "launch analysis and delete errors on redacted method" in withWebDriver { implicit driver =>
  ...
  "owner can abort a launched submission" in withWebDriver { implicit driver =>
    ...
    implicit val authToken: AuthToken = user.makeAuthToken()
    withCleanBillingProject(user) { billingProject =>
      withWorkspace(billingProject, "MethodRedactedSpec_abort_submission") { workspaceName =>
        ...
        withMethod("MethodRedactedSpec_abort", MethodData.SimpleMethod) { methodName =>
          api.methodConfigurations.createMethodConfigInWorkspace(billingProject, workspaceName,
            MethodData.SimpleMethod.copy(methodName = methodName), SimpleMethodConfig.configNamespace, methodName, 1,
